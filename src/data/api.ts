...
import { api as realApi } from "./apiReal";
    ...
    consume: async (partId: string, quantity: number) => {
      ...
      if (part) {
        if (part.quantity < quantity)
          throw new Error(`Not enough stock for part ${part.name}`);
  ...
  workLogs: {
    ...
    stopWork: async (logId: string) => {
      ...
      if (log && !log.endTime) {
        ...
        if (order) {
          order.totalWorkTimeMinutes =
            (order.totalWorkTimeMinutes || 0) + log.durationMinutes;
        }
      }
      return log;
    },
  ...
  warehouse: {
    acceptDelivery: async (items: { partId: string; quantity: number }[]) => {
      ...
      for (const item of items) {
        ...
        const pendingOrders = mockPartOrders.filter(
          (po) => po.sparePartId === item.partId && po.status === "ORDERED",
        );
        ...
        for (const po of pendingOrders) {
          ...
          if (part && part.quantity >= po.quantity) {
            ...
            if (po.repairOrderId) {
              ...
              const allRepairPartOrders = mockPartOrders.filter(
                (o) => o.repairOrderId === po.repairOrderId,
              );
              const allDelivered = allRepairPartOrders.every(
                (o) => o.status === "DELIVERED" || o.status === "CANCELLED",
              );

              if (allDelivered) {
                const repairOrder = mockOrders.find(
                  (o) => o.id === po.repairOrderId,
                );
