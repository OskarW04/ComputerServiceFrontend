...
export default function WarehouseInventory() {
  ...
  const handleAddPart = async () => {
    ...
    setNewPart({ name: '', category: '', quantity: '', minQuantity: '', price: '' });
  ...
  const orderColumns: ColumnDef<PartOrder>[] = [
      ...
      },
      {
          id: "actions",
          cell: ({ row }) => {
              if (row.original.status === 'ORDERED' || row.original.status === 'IN_DELIVERY') {
                  return <Button size="sm" onClick={() => handleReceiveDelivery(row.original.id)}>Przyjmij</Button>
              }
              return null;
          }
      }
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="space-x-2">
            <Button variant="secondary" onClick={() => navigate('/warehouse/delivery')}>Przyjmij Dostawę</Button>
            <Dialog open={orderPartsOpen} onOpenChange={setOrderPartsOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>Złóż zamówienie na nowe części od dostawcy.</DialogDescription>
                </DialogHeader>
                <div className="grid gap-4 py-4">
                  <div className="grid gap-2">
                    ...
                    <select
                      ...
                      onChange={(e) => setNewOrder({...newOrder, sparePartId: e.target.value})}
                    >
                ...
                <DialogFooter>
                  <Button variant="outline" onClick={() => setOrderPartsOpen(false)}>Anuluj</Button>
            ...
            <Dialog open={addPartOpen} onOpenChange={setAddPartOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>Dodaj nową część zamienną do magazynu.</DialogDescription>
                </DialogHeader>
                <div className="grid gap-4 py-4">
                  <div className="grid gap-2">
                ...
                <DialogFooter>
                  <Button variant="outline" onClick={() => setAddPartOpen(false)}>Anuluj</Button>
        ...
        <DialogContent>
            ...
            <div className="py-4 space-y-4">
                <div className="space-y-2">
                    <Label>Część</Label>
                    <Input value={parts.find(p => p.id === releaseData.partId)?.name || ''} disabled />
                </div>
                <div className="space-y-2">
                    <Label>Ilość</Label>
                    <Input type="number" value={releaseData.quantity} onChange={e => setReleaseData({...releaseData, quantity: e.target.value})} />
                </div>
            </div>
            <DialogFooter>
        ...
        <TabsContent value="inventory">
          <Card>
            <CardHeader>
                ...
                <CardDescription>Zarządzaj stanem magazynowym części zamiennych.</CardDescription>
            </CardHeader>
            <CardContent>
              <DataTable columns={partColumns} data={parts} searchKey="name" searchPlaceholder="Szukaj części..." />
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="orders">
          <Card>
            <CardHeader>
                ...
                <CardDescription>Śledź status zamówień nowych części.</CardDescription>
            </CardHeader>
            <CardContent>
                <DataTable columns={orderColumns} data={orders} searchKey="id" searchPlaceholder="Szukaj zamówienia..." />
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>
