...
export default function ClientOrderHistory() {
  ...
  const columns: ColumnDef<RepairOrder>[] = [
    ...
    },
    {
        ...
        cell: ({ row }) => {
            return (
                <Button variant="ghost" size="sm" onClick={() => navigate(`/client/orders/${row.original.id}`)}>
                    Szczegóły
                </Button>
            )
        }
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <DataTable columns={columns} data={orders} searchKey="orderNumber" searchPlaceholder="Szukaj po numerze..." />
        </CardContent>
      </Card>
    </div>
  );
